...
public sealed class IngredientUpdateTests
{
    ...
    private static readonly Category DefaultCategory = Category.Create(
        new CategoryId(Guid.NewGuid()), "Spices", 1, CategoryType.Ingredient);
    ...
    public void Update_Should_UpdateProperties_When_InputIsValid()
    {
        ...
        decimal newPrice = 10.0m;
        Category newCategory = Category.Create(new CategoryId(Guid.NewGuid()), "Herbs Category", 2, CategoryType.Ingredient);
        Collection<Category> newCategories = [newCategory];

        // Act
        ingredient.Update(newName, newDescription, newPrice, newCategories);
        ...
        Assert.Equal(newCategory.Id, ingredient.Categories[0].Id);
    }
    ...
    public void Update_Should_ThrowDomainException_When_CategoriesContainWrongType()
    {
        // Arrange
        Ingredient ingredient = CreateIngredient();
        Category invalidCategory = Category.Create(new CategoryId(Guid.NewGuid()), "ProdCat", 1, CategoryType.Product);
        Collection<Category> categories = [invalidCategory];

        // Act & Assert
        DomainException exception = Assert.Throws<DomainException>(() => UpdateIngredient(ingredient, categories: categories));
