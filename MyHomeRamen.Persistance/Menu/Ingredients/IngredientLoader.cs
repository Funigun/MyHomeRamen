...
{
    async Task<Ingredient> IIngredientLoader.ById(IngredientId ingredientId, CancellationToken cancellationToken)
        => await menuDbContext.Ingredients.Include(i => i.Categories)
                                          .AsSplitQuery()
                                          .FirstAsync(i => i.Id == ingredientId, cancellationToken);

    async Task<IEnumerable<Ingredient>> IIngredientLoader.ByIds(IEnumerable<IngredientId> ingredientIds, CancellationToken cancellationToken)
