...
public partial class ShoppingCartDbContext : IIngredientRepository
{
    public void Add(Ingredient entity) => Ingredients.Add(entity);

    public void AddRange(IEnumerable<Ingredient> entities) => Ingredients.AddRange(entities);

    public async Task<bool> Exists(Expression<Func<Ingredient, bool>> predicate, CancellationToken cancellationToken)
        => await Ingredients.AnyAsync(predicate, cancellationToken);

    public void Delete(Ingredient entity) => Ingredients.Remove(entity);
