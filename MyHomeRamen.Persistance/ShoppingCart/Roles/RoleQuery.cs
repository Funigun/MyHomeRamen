...
public partial class RoleRepository : IRoleQuery
{
    async Task<Role?> IRoleQuery.ByIdAsync(RoleId roleId, CancellationToken cancellationToken)
        => await shoppingCartDbContext.Roles.AsNoTracking().FirstOrDefaultAsync(role => role.Id == roleId, cancellationToken);

    public async Task<Role?> GetByNameWithPermissionsAsync(string name, CancellationToken cancellationToken)
        => await shoppingCartDbContext.Roles.AsNoTracking()
