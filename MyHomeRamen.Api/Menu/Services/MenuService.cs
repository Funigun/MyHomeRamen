...
namespace MyHomeRamen.Api.Menu.Services;

public sealed class MenuService(IMenuDbContext dbContext) : IMenuService
{
    ...
    {
        bool exists = await dbContext.Products.Exists(p => p.Id == (ProductId)productId, cancellationToken);
        if (!exists)
        {
            return false;
        }
