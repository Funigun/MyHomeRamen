...
public sealed class DeleteIngredientEndpoint : IEndpoint
{
    ...
    {
        endpointBuilder.MapStandardValidatedDelete<DeleteIngredientCommand>("api/menu/ingredients/{id}", HandleAsync)
    ...
    private static async Task<IResult> HandleAsync(DeleteIngredientCommand id, [FromServices] IRequestHandler<DeleteIngredientCommand, IResult> handler, CancellationToken cancellationToken)
    {
