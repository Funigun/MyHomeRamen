...
- **POS-009**: **No audit / metadata leakage**: DAOs contain only the data needed for the use case. They do not include `AuditableEntity` fields (CreatedBy, CreatedOn, ModifiedBy, ModifiedOn, RestaurantId) or other entity metadata that would be loaded by default, so these values cannot leak into the cache or API responses by accident.
