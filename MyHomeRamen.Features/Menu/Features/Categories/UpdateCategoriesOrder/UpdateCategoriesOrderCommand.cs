...
public sealed class UpdateCategoriesOrderAuthorizationPolicy(ICurrentUser currentUser) : IAuthorizationPolicy<UpdateCategoriesOrderCommand>
{
    public async Task<bool> Authorize(UpdateCategoriesOrderCommand request, CancellationToken cancellationToken)
    {
        return currentUser.CanEditCategory();
    }
}
