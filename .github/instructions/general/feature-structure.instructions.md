...
Structure that lead to Feature folder will be defined by instructions for specific layer/project e.g. `api-layer.instructions.md` for API layer.
...
- Basic validations should be implemented by creating `AbstractValidator<T>` implementation in `MyHomeRamen.Common.Contracts` project to be reusable across modules, features and projects (API, Blazor)
