...
- Exposes one internal method: `Validate{Entity}({Entity} entity)`
...
### 2.1) DbContext
- Each module has interface in `MyHomeRamen.Domain/{Module}/Database/` implemented in `MyHomeRamen.Persistance/{Module}/`
- All module `DbContext` inherit from `BaseDbContext`
