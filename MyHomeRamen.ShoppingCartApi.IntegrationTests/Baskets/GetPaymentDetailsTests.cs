...
public sealed class GetPaymentDetailsTests(WebApiFactory apiFactory) : IClassFixture<WebApiFactory>
{
    ...
    public async Task GetPaymentDetails_ShouldReturnOk_ForBasketWithPaymentDetails()
    {
        ...
        Basket basket = DataGenerator.CreateBasket([DataGenerator.CreateBasketItem(product)], new UserId(userId), paymentDetails: paymentDetails);
    ...
    {
        UserId customeId = Guid.CreateVersion7();
        UserId guestId = Guid.CreateVersion7();
        ...
        Basket? basket = DataGenerator.CreateBasket([basketItem], new UserId(customeId));
        ...
        using HttpClient client = apiFactory.CreateClient();
        using HttpRequestMessage httpRequest = HttpClientExtensions.CreateGetMessage(url);
        httpRequest.AddAuthorizationHeader(UserRoles.Customer, guestId.Value.ToString());
